...
export function ProgressTree({
  ...
  return (
    <div id="sidebar-stages" className="space-y-3">
      ...
      {roadmapData.stages
        .filter((stage: RoadmapStage) => !hasScenarios || !stage.scenarioSpecific || stage.scenarioSpecific === selectedScenario)
        .map((stage: RoadmapStage, sIdx: number) => {
          ...
          const originalStageIdx = roadmapData.stages.findIndex(s => s.id === stage.id);
          const isActiveStage = state.currentStage === originalStageIdx;
          const isExpanded = expandedStages[originalStageIdx];

          const visibleSteps = stage.steps.filter((s: RoadmapStep) => 
            !hasScenarios || !s.scenarioSpecific || s.scenarioSpecific === selectedScenario
          );
          const completedInStage = visibleSteps.filter((s: RoadmapStep) =>
            state.completedSteps.has(s.id),
          ).length;
          const stageProgress = visibleSteps.length > 0
