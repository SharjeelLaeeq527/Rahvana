    ...
    <div className="space-y-4">
        ...
        {step.options!.map((opt) => {
          ...
          return (
            <button key={opt.value}
              ...
              className={`w-full text-left p-4 rounded-2xl transition-all duration-200 flex items-center justify-between gap-4 border-2 cursor-pointer active:scale-[0.99] ${isSel ? (isPassOpt ? "border-emerald-500 bg-emerald-50" : "border-red-400 bg-red-50") : "border-slate-100 bg-white hover:border-slate-200 hover:bg-slate-50"}`}
            >
              <span className={`text-[14px] font-semibold flex-1 ${isSel ? (isPassOpt ? "text-emerald-700" : "text-red-700") : "text-slate-800"}`}>{opt.label}</span>
              {isSel && (
      ...
      {step.sourceUrl && (
        <a href={step.sourceUrl} target="_blank" rel="noopener noreferrer"
          ...
          <ExternalLink size={11} /><span>{step.subtitle}</span>
        </a>
      )}
    </div>
  );
    ...
    <div className="w-full max-w-2xl mx-auto p-4 sm:p-8">
      <div className="bg-white rounded-[2.5rem] p-10 text-center shadow-2xl shadow-slate-200/50 border-2 border-slate-100/50">
        ...
        <p className="text-slate-500 text-[15px] leading-relaxed mb-10 max-w-md mx-auto">
          We are working on adding detailed visa guidance for {destination}. Currently this tool specializes in <strong>United States</strong> immigration.
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
...
function ResultsScreen({ answers, results, ineligibleCodes, onReset }: {
  answers: VisaExplorationAnswers; results: any[]; ineligibleCodes: string[]; onReset: () => void;
}) {
    ...
    <div className="w-full max-w-4xl mx-auto p-4 sm:p-8">
      <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-slate-200/60 border-2 border-slate-100/50 overflow-hidden flex flex-col mt-12 md:mt-2">
        ...
        <div className="p-8 sm:p-10 bg-slate-50/80 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          ...
          <button onClick={onReset} className="px-5 py-3 rounded-xl bg-white shadow-sm border border-slate-200 text-slate-600 font-bold text-sm hover:bg-slate-50 transition-all">
            Start Over
          </button>
        </div>
        ...
        {ineligibleCodes.length > 0 && (
          <div className="px-8 sm:px-10 pb-6">
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200">
              <p className="text-[11px] font-black uppercase tracking-widest text-slate-400 mb-3">Did Not Qualify For</p>
...
export default function VisaExplorationTool() {
  ...
  if (showResults) {
    return (
      <div className="w-full min-h-[80vh] flex items-center justify-center bg-transparent">
        <ResultsScreen answers={answers} results={eligibleVisas} ineligibleCodes={ineligibleCodes} onReset={handleReset} />
      </div>
    );
  }
        ...
        <div className="p-8 sm:p-10 pb-0 shrink-0">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            ...
            <div className="flex items-center gap-2">
              <span className="text-[12px] font-black uppercase tracking-widest text-slate-400">Step {stepIndex + 1}</span>
          ...
          <div>
            {isLastStep && gatesDone ? (
              ...
              <button onClick={handleNext} disabled={!currentStep?.canProceed}
                className={`py-4 px-10 rounded-2xl font-black uppercase tracking-widest text-[13px] transition-all flex items-center gap-3 ${currentStep?.canProceed ? "bg-[#0D6E6E] text-white shadow-xl shadow-[#0D6E6E]/20 hover:bg-[#095555]" : "bg-slate-200 text-slate-400 cursor-not-allowed"}`}>
